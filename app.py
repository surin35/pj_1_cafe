...
def load_data():
    ...
    # 종사자 데이터 로드 및 전처리 (UTF-8 인코딩)
    work_df = pd.read_csv(work_path, encoding='utf-8')
...
try:
    ...
    with tab5:
        ...
        # 좌표 컬럼 확인 및 변환
        # cafe_data_merge_v2.csv의 경우:
        # Index 26: 좌표정보(x) (EPSG:5174)
        # Index 27: 좌표정보(y) (EPSG:5174)
        # 위경도 컬럼이 비어있다면 변환 로직 적용
        ...
        valid_map_df = df.dropna(subset=[col_x, col_y]).sample(n=min(2000, len(df)))
        
        if not valid_map_df.empty:
            # 좌표 변환 (EPSG:5174 -> EPSG:4326)
            transformer = Transformer.from_crs("epsg:5174", "epsg:4326", always_xy=True)
            
            lon, lat = transformer.transform(valid_map_df[col_x].values, valid_map_df[col_y].values)
            valid_map_df['경도'] = lon
            valid_map_df['위도'] = lat
            
            # 유효한 위경도 범위 필터링 (서울 기준)
            # 위도 37.4 ~ 37.7, 경도 126.7 ~ 127.2
            mask_map = (valid_map_df['위도'] > 37.4) & (valid_map_df['위도'] < 37.7) & \
                       (valid_map_df['경도'] > 126.7) & (valid_map_df['경도'] < 127.2)
            final_map_df = valid_map_df[mask_map]
            
            st.map(final_map_df[['위도', '경도']])
        else:
            st.warning("지도 시각화에 필요한 좌표 데이터가 부족합니다.")
